...
public class SocialConnectionRepository implements ConnectionRepository {
    ...
    public Connection<?> getConnection(final ConnectionKey connectionKey) {
        final Query query = query(
                where("userId").is(userId).and("providerId").is(connectionKey.getProviderId()).and("providerUserId")
    ...
    public void addConnection(final Connection<?> connection) {
        try {
            final SocialConnection socialConnection = mongoConnectionConverters.fromConnection(userId)
                    .apply(connection);
    ...
    public void updateConnection(final Connection<?> connection) {
        ...
        final Query query = query(
                where("userId").is(userId).and("providerId").is(socialConnection.getProviderId()).and("providerUserId")
                        .is(socialConnection.getProviderUserId()));
        final Update update = update("displayName", socialConnection.getDisplayName())
    ...
    private List<Connection<?>> findConnections(Query query) {
        return mongo.find(query, SocialConnection.class).stream().map(mongoConnectionConverters.toConnection())
                .collect(toList());
    }
